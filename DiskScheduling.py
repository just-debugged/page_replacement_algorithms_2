...
def Main():
    ...
    title = Label(Menu, text="Disk Scheduling GUI", anchor=CENTER, bd=12, padx=200, bg="#1A1C20", fg=mainbg,
                  font=("Century Gothic", 30), pady=2).grid(row=0)
    ...
    E1 = Entry(Menu, textvariable=Start, bd=0, fg=TextCol, width=8, bg=eleBG, justify=CENTER,
               font=("Century Gothic", 16))  # Textbox
    ...
    B1 = Button(Menu, borderwidth=0, padx=20, pady=10, bg=eleBG, fg=TextCol, text="Visualise", \
                command=lambda: Visualise(Option.get(), user_inp.get(1.0, END), Start.get()),
                font=("Century Gothic", 12))  # Button
    ...
    B3 = Button(Menu, borderwidth="0", text="Back", bg=eleBG, fg=TextCol, font=("Century Gothic", 12),
                command=Menu.destroy, padx=20, pady=10)
